...
//        textSize(12);
//
//        // KORRIGIERTE Logbuch-Anzeige (Simulation des Scrollens fehlt, da logMessage String ist)
//        // Wir verwenden die String-Version, die die neuesten Nachrichten hinzufügt.
//        // HINWEIS: Bei sehr langen Spielen wird dieser Kasten überlaufen, da es ein String ist.
...
//            finalizeLearning(winnerId); // <--- HIER LERNFINALISIERUNG
//            return;
//        }
//
//        // 7. Nächster Spieler
...
//     */
//    private void finalizeLearning(String winnerId) {
//        for (Map.Entry<String, ReflexAgent> entry : agents.entrySet()) {
//            ReflexAgent agent = entry.getValue();
//            double reward;
//
//            // Belohnung: +1000 für Sieg, -1000 für Niederlage
//            if (agent.getPlayerId().equals(winnerId)) {
//                reward = 1000.0;
//            } else {
//                // Hier könnte man genauer unterscheiden (z.B. -1000 für Verlierer, 0 für Dritte)
//                reward = -1000.0;
//            }
//
//            // Führt den Lernschritt aus (updateWeights simuliert TD-Lernen)
//            agent.updateWeights(reward);
//            logMessage += "\nKI " + agent.getPlayerId() + " beendet Lernen. Belohnung: " + reward;
//        }
//    }
//
//
...
/**
 * Santorini GUI (Processing) – überarbeitete Version mit korrekter Startspieler- und Platzierungs-Logik.
 * Benötigt: Board.java, Worker.java, Move.java, ReflexAgent.java, SmartAgent.java
 *
 * **WICHTIGE ÄNDERUNGEN FÜR LERNFÄHIGE KI (SmartAgent):**
 * 1. Der Agenten-Typ wird von ReflexAgent zu SmartAgent geändert.
 * 2. Die Methode setupGameStructure() instanziiert nun SmartAgent.
 * 3. Die Methode finalizeLearning() wird angepasst, um notifyGameEnd(reward) des SmartAgent zu verwenden.
 */
public class SantoriniGUI extends PApplet {
    ...
    // --- Spiel-Status ---
    private Board board;
    // ÄNDERUNG: Von ReflexAgent zu SmartAgent
    private Map<String, SmartAgent> agents;
    private List<String> playerIds;
    private int currentPlayerIndex = 0;
    private String currentPlayerId = null;
    private String startPlayerId = null; // gewählter Startspieler
    ...
    public void setupGameStructure(int opponents) {
        this.playerIds = new ArrayList<>();
        // ÄNDERUNG: HashMap mit SmartAgent
        this.agents = new HashMap<>();
        this.playerIds.add("P1");

        if (opponents == 1) {
            this.playerIds.add("P2");
            // ÄNDERUNG: SmartAgent instanziieren
            agents.put("P2", new SmartAgent("P2"));
        } else if (opponents == 2) {
            this.playerIds.add("P2");
            this.playerIds.add("P3");
            // ÄNDERUNG: SmartAgent instanziieren
            agents.put("P3", new SmartAgent("P3"));
        }
    ...
    private void runAiTurn() {
        ...
        // Agent-Typ ist jetzt SmartAgent
        SmartAgent agent = agents.get(currentPlayerId);
        ...
        if (move == null) {
            gameOver = true;
            // Der Gewinner ist der Spieler VOR dem blockierten Spieler
            String winnerIdIfBlocked = playerIds.get((currentPlayerIndex - 1 + playerIds.size()) % playerIds.size());
            winnerId = winnerIdIfBlocked;
            moveEvaluation = currentPlayerId + " ist blockiert und verliert.";
     ...
     */
    private void finalizeLearning(String winnerId) {
        // ÄNDERUNG: Iteriere über SmartAgent und rufe notifyGameEnd auf
        for (Map.Entry<String, SmartAgent> entry : agents.entrySet()) {
            SmartAgent agent = entry.getValue();
            double reward;
            ...
            // Benachrichtigt den SmartAgent, der die Gewichte aktualisiert und speichert
            agent.notifyGameEnd(reward);
            logMessage += "\nKI " + agent.getPlayerId() + " beendet Lernen. Belohnung: " + reward;
        }
    }
